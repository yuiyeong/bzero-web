/**
 * Supabase 인증 에러 처리 유틸리티
 */
import { AuthError } from "@supabase/supabase-js";

/**
 * Supabase AuthError 코드를 한국어 메시지로 매핑
 *
...
const AUTH_ERROR_MESSAGE_MAP: Record<string, string> = {
  ...
  weak_password: "비밀번호가 너무 약합니다. 8자 이상으로 작성해주세요.",
...
/**
 * Supabase AuthError를 사용자 친화적인 한국어 메시지로 변환
 *
...
export function generateAuthErrorMessage(error: Error): string {
  if (error instanceof AuthError && error.code) {
    return AUTH_ERROR_MESSAGE_MAP[error.code] || error.message;
  }
